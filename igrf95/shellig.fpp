...
c Changed from CALL FELDI(XI,H); XI, H are in COMMON block; results
c are the same; dkb Feb 1998
      CALL FELDI
      Q=H(1)/RQ                                                         
      DX=H(3)+H(3)+Q*XI(1)                                              
      DY=H(4)+H(4)+Q*XI(2)                                              
...
C			BE CALCULATED (e.g.:1995.5 for day 185 of 1995)
...
C ### changed to conform with IGRF 45-95, also FILMOD, DTEMOD arrays +1
	DATA		FILMOD /'dgrf45.dat', 'dgrf50.dat',            
     ...
     3			'dgrf85.dat', 'dgrf90.dat', 'igrf95.dat',
     4			'igrf95s.dat'/
...
C-- DETERMINE IGRF-YEARS FOR INPUT-YEAR
	...
	DTE1 = DTEMOD(L)   
	FIL1 = FILMOD(L)   
	write(path1,104)FILMOD(L)
104	format(DATAPATH,A11)
	DTE2 = DTEMOD(L+1) 
	FIL2 = FILMOD(L+1) 
	write(path2,105)FILMOD(L+1)
105	format(DATAPATH,A11)
C-- GET IGRF COEFFICIENTS FOR THE BOUNDARY YEARS
cpguio	CALL GETSHC (IU, FIL1, NMAX1, ERAD, GH1, IER)  
	CALL GETSHC (IU, path1, NMAX1, ERAD, GH1, IER)  
	    IF (IER .NE. 0) STOP                           
cpguio	CALL GETSHC (IU, FIL2, NMAX2, ERAD, GH2, IER)  
	CALL GETSHC (IU, path2, NMAX2, ERAD, GH2, IER)  
	    IF (IER .NE. 0) STOP                    
